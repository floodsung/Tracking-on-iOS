...
public:
	...
	template <typename T2>
	friend std::ostream& operator <<(std::ostream &rOS, const StruckRect<T2>& rStruckRect);
...
typedef StruckRect<int> IntStruckRect;
typedef StruckRect<float> FloatStruckRect;

template <typename T>
std::ostream& operator <<(std::ostream &rOS, const StruckRect<T>& rStruckRect)
{
	rOS << "[origin: (" << rStruckRect.m_xMin << ", " << rStruckRect.m_yMin << ") size: (" << rStruckRect.m_width << ", " << rStruckRect.m_height << ")]";
	return rOS;
}
