...
public:
	...
	int Sum(const IntStruckRect& rStruckRect, int channel = 0) const;
	void Hist(const IntStruckRect& rStruckRect, Eigen::VectorXd& h) const;
	
	inline const cv::Mat& GetImage(int channel = 0) const { return m_images[channel]; }
	inline const IntStruckRect& GetStruckRect() const { return m_StruckRect; }
