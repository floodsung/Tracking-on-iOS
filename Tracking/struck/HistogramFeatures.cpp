...
{
	IntStruckRect StruckRect = s.GetROI(); // note this truncates to integers
	//cv::StruckRect roi(StruckRect.XMin(), StruckRect.YMin(), StruckRect.Width(), StruckRect.Height());
	...
	for (int il = 0; il < kNumLevels; ++il)
	{
		...
		FloatStruckRect cell(0.f, 0.f, w, h);
