...
double LaRank::Evaluate(const Eigen::VectorXd& x, const FloatStruckRect& y) const
{
	double f = 0.0;
...
{
	const FloatStruckRect& centre(sample.GetStruckRects()[0]);
	vector<VectorXd> fvs;
	...
	for (int i = 0; i < (int)fvs.size(); ++i)
	{
		...
		FloatStruckRect y(sample.GetStruckRects()[i]);
...
void LaRank::Update(const MultiSample& sample, int y)
{
	...
	const vector<FloatStruckRect>& StruckRects = sample.GetStruckRects();
	FloatStruckRect centre = StruckRects[y];
	for (int i = 0; i < (int)StruckRects.size(); ++i)
	{
		// express r in coord frame of centre sample
		FloatStruckRect r = StruckRects[i];
		...
		if (!m_config.quietMode && m_config.debugMode)
		{
			...
			IntStruckRect StruckRect = StruckRects[i];
			cv::Rect roi(StruckRect.XMin(), StruckRect.YMin(), StruckRect.Width(), StruckRect.Height());
			cv::resize(sample.GetImage().GetImage(0)(roi), im, im.size());
			sp->images.push_back(im);
		}
	}
	// evaluate features for each sample
	sp->x.resize(StruckRects.size());
